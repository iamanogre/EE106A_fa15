...
def inverse_g(g):
    ...
    p = g[0:3, 3]
    p_inv = -1*np.dot(rot_inv, p)
    p_inv = np.array([p_inv])
    top = np.concatenate((rot_inv, p_inv.T), axis=1)
...
def exp_to_quaternion(omega, theta):
    q_3 = np.array([np.cos(theta/2)])
    q = omega*np.sin(theta/2)
    #print(q_3)
    #print(q)
    q = np.concatenate((q, q_3), axis = 0)
    #print(q)
    return q
...
def create_rbt(omega, theta, trans):
    ...
    bot_row = [0,0,0,1]
    bot_row = np.array([bot_row])
    g = np.concatenate((g,bot_row), axis=0)
    return g
...
def find_omega_theta(R):
    ...
    t = [(R[2,1]-R[1,2]),(R[0,2]-R[2,0]),(R[1,0]-R[0,1])]
    omega = np.array(t)
...
def find_v(omega, theta, trans):
    ...
    """    
    #YOUR CODE HERE
    omega_nd = np.array([omega])
    omega_hat = kfs.skew_3d(omega)
    omega_T = np.transpose(omega_nd)
    rot_mat = kfs.rotation_3d(omega,theta)

    I = np.eye(3)
    
    if False in np.equal(rot_mat, I):

        term1 = np.dot( (I-rot_mat) ,omega_hat )
        term2 = theta*np.dot(omega_T,omega_nd)
        A = term1+term2
