...
def findDesiredTheta(x1, y1, x2, y2):
    """ 
...
class Controller:
    ...
    # gets photo data and sends a message to the Explore Service. Reacts to response from service
    ...
    def run(self, hard_stop):
        ...
        des_theta = findDesiredTheta(self.x, self.y, self.x_des, self.y_des)
        ...
        if (facingCorrectWay):
            ...
            #    print("go forward")
                ...
                self.twist.angular.z = rotationDirection * 0.04 *rotate#0.06 # Makes it rotate slightly as it's moving 
            else:
                #Stop moving
                self.twist.linear.x = 0
                self.twist.angular.z = rotationDirection * 0 * rotate
                self.finished = True

        else:
            #If not facing correct way then rotate in the appropriate direction
            self.twist.angular.z = rotationDirection * 0.18 * rotate#0.21

        #Print out information
        print(str(ret.updated) + "\tposition: " + str([self.x, self.y]) + "\ttheta: " + str(self.theta*180/np.pi) + "\tdesired " + str(des_theta*180/np.pi) + "\tdes_position: " + str([self.x_des, self.y_des]))

        # publish twist
        if (hard_stop):
            self.pub.publish(ZeroTwist)
        else:
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Use: controller.py zumy## exploreService")
        sys.exit()
    ...
    if (zumy == 'zumy5c' or zumy == 'zumy3b'):
        rotate = -1
    else:
        rotate = 1

    node = Controller(zumy, exploreService)
